...
namespace AdvancedSharpAdbClient.DeviceCommands.Models.Tests
{
    ...
    public class AreaTests
    {
        ...
        public void NonDefaultConstructorTest(int x, int y, int width, int height)
        {
            ...
            Area rect2 = new Rectangle(new Point(x, y), new Size(width, height));
        ...
        public void DimensionsTest(int x, int y, int width, int height)
        {
            ...
            Assert.Equal(new Size(width, height), rect.Rectangle.Size);
            ...
            Size s = new(x, y);
            rect.Location = p;
            rect.rectangle.Size = s;

            Assert.Equal(p, rect.Location);
            Assert.Equal(s, rect.Rectangle.Size);
        ...
        {
            Size size = new(x, y);
            Area rect = new Rectangle(10, 10, 10, 10)
            {
                Size = size
            };
            Assert.Equal(size, rect.Rectangle.Size);
        ...
        public void AreaFConversionTest(float x, float y, float width, float height)
        {
            ...
            Assert.Equal<Area>(rCeiling, Rectangle.Ceiling(rect));
            Assert.Equal<Area>(rTruncate, Rectangle.Truncate(rect));
            Assert.Equal<Area>(rRound, Rectangle.Round(rect));
        }
