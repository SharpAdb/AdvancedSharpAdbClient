...
#else
                    await Utilities.Run(() => Inner.Read(buffer, offset + 1, count - 1), cancellationToken).ConfigureAwait(false);
#endif
                read++;
                pendingByte = null;
            }
...
#else
                    await Utilities.Run(() => Inner.Read(buffer, offset, count), cancellationToken).ConfigureAwait(false);
#endif
            }
