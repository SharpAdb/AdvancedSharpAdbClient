...
namespace AdvancedSharpAdbClient
{
    public partial interface ISyncService
    {
        ...
        Task PushAsync(Stream stream, string remotePath, int permissions, DateTimeOffset timestamp, Action<SyncProgressChangedEventArgs>? callback, CancellationToken cancellationToken);
        ...
        public interface IWinRT
        {
            ...
            /// <returns>A <see cref="Task"/> which represents the asynchronous operation.</returns>
            /// <remarks>The <paramref name="permissions"/> should coverts to a decimal number. For example, <c>644</c> should be <c>420</c> in decimal, <c>&amp;O644</c> in VB.NET and <c>0o644</c> in F# and Python.</remarks>
            Task PushAsync(IInputStream stream, string remotePath, int permissions, DateTimeOffset timestamp, Action<SyncProgressChangedEventArgs>? callback, CancellationToken cancellationToken);
